...
-(void)radioTappped:(id)sender{
    VCRadioButton *caller = (VCRadioButton*)sender;
...
//    [self removeObserver:self forKeyPath:kObserver];
    ...
    @catch (NSException *exception) {
        NSLog(@"exception in removing the the radio observer  = %@",exception);
    }
    @finally {
...
- (void)drawRect:(CGRect)rect{
    UIImage * normalImge = [UIImage imageNamed:@"rd_normal"];
    
    CGSize ghostSize = CGSizeMake(rect.size.width, rect.size.width);
    ...
    CGAffineTransform flipThenShift = CGAffineTransformTranslate(flip,0,-rect.size.width);
    CGContextConcatCTM(context, flipThenShift);
    
    CGContextDrawImage(context, rect, [normalImge CGImage]);
    
    CGContextSetBlendMode(context, kCGBlendModeSourceAtop);
    CGContextSetAlpha(context,0.5);
    CGRect transformedGhostRect = CGRectApplyAffineTransform(ghostRect, flipThenShift);
    CGContextDrawImage(context, transformedGhostRect, [normalImge CGImage]);
